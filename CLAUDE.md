...
Next up (the agent milestone is done — see "Agent invariants" under Architecture; the `mua.api` bridge's options slice and Steps 1–3 — value marshaling + `mua.g`, `mua_register_tool`, and the autocmd event set — have landed):

1. **`mua.api` + Lua bridge (continued)** — the identity feature. *Landed:* the global API seam (`src/mua/api/global.c` with `FUNC_API_SINCE`, the table-driven `mua.api.mua_*` registration in `src/mua/lua/bridge.c`, the C-side options store in `src/mua/options.c`, the `mua.o` options sugar in `runtime/lua/mua/`), **Step 1** — Lua↔Object value marshaling (`object_to_lua`/`lua_pop_object`, shared `api_copy_object`/`api_free_object`) with the `mua.g` variable store (`src/mua/variables.c`) and sugar; **Step 2** — `mua_register_tool` (user-defined tools from Lua) with the `cjson_to_object`/`object_to_cjson` converter (`src/mua/json.c`) and the dynamic registry (`src/mua/tools.c`); and **Step 3** — the autocmd event set (`mua_create_autocmd`) with the core store (`src/mua/autocmd.c`), dispatched from main.c at the agent's existing callback seams. *Sequence to done* (dependency-ordered; full breakdown in [docs/api-bridge.md](docs/api-bridge.md)):
