...
Next up (the agent milestone is done — see "Agent invariants" under Architecture; the `mua.api` bridge's first slice — `mua.o` options — has landed):

1. **`mua.api` + Lua bridge (continued)** — the identity feature. *Landed:* the global API seam (`src/mua/api/global.c` with `FUNC_API_SINCE`, the table-driven `mua.api.mua_*` registration in `src/mua/lua/bridge.c`, the C-side options store in `src/mua/options.c`, and the `mua.o` options sugar in `runtime/lua/mua/`). *Still ahead:* `mua_register_tool` from Lua, the autocmd event set (`SessionStart`/`SessionEnd`/`ToolPre`/`ToolPost`/`StreamDelta`), session-scoped `mua_sess_*`, `mua.g`, and the `--embed` msgpack-RPC server the table-driven dispatch was shaped for.
