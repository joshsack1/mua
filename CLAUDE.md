...
Next up (the agent milestone is done — see "Agent invariants" under Architecture; the `mua.api` bridge's options slice and Step 1 — value marshaling + `mua.g` — have landed):

1. **`mua.api` + Lua bridge (continued)** — the identity feature. *Landed:* the global API seam (`src/mua/api/global.c` with `FUNC_API_SINCE`, the table-driven `mua.api.mua_*` registration in `src/mua/lua/bridge.c`, the C-side options store in `src/mua/options.c`, the `mua.o` options sugar in `runtime/lua/mua/`), and **Step 1** — Lua↔Object value marshaling (`object_to_lua`/`lua_pop_object`, shared `api_copy_object`/`api_free_object`) with the `mua.g` variable store (`src/mua/variables.c`) and sugar. *Sequence to done* (dependency-ordered; full breakdown in [docs/api-bridge.md](docs/api-bridge.md)):
   ...
   2. **`mua_register_tool`** — a dynamic tool registry beside the static `tool_defs[]`; a Lua callback wrapped as a synchronous `ToolExecuteFn` (fires `done` inline). The `nvim_create_user_command` analog.
   3. **Autocmd event set** — `mua_create_autocmd` over `SessionStart`/`SessionEnd`/`ToolPre`/`ToolPost`/`StreamDelta`, dispatched at the C callback seams that already exist; `ToolPre` is the one that wraps the gate chokepoint and can refuse.
   4. **Session scope `mua_sess_*`** — a `Session` handle table (`0` = current) over the existing session store; read accessors (`mua_sess_get_messages`) first.
